  ...
  const startAutoRotate = useCallback(() => {
    ...
    const step = () => {
      if (!draggingRef.current && !inertiaRAF.current && !openingRef.current) {
        rotationRef.current.y = wrapAngleSigned(rotationRef.current.y - autoRotateSpeed); // Speed of auto-rotation
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      }
          ...
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  <div
                    className="item__image absolute block overflow-hidden cursor-pointer transition-transform duration-300 group hover:border-white/30"
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    <img
                      ...
                      draggable={false}
                      alt={it.alt}
                      className="w-full h-full object-contain p-6 pointer-events-none transition-transform duration-500 group-hover:scale-110"
                      style={{
                        backfaceVisibility: 'hidden',
